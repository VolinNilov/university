...
from app.resources import DEFAULT_PARAMS
...
class AppController(QObject):
    ...
    def on_animate_clicked(self):
        ...
        if self.animation_data:
            print("[Controller] Попытка запуска анимации...")
            params = self.view.get_parameters()
            radius = params['radius']
            print(f"[Controller] Передача данных в анимацию. Радиус: {radius}")
            # Проверим содержимое animation_data перед передачей
            if isinstance(self.animation_data, dict) and 'x' in self.animation_data and 'y' in self.animation_data:
                print(f"[Controller] Передается: x ({len(self.animation_data['x'])} точек), y ({len(self.animation_data['y'])} точек)")
            else:
                print("[Controller] ОШИБКА: Передаваемые данные некорректны!")
            ...
            print("[Controller] ОШИБКА: Нет данных для анимации!")
    ...
    def on_animation_data_ready(self, data):
        ...
        print(f"[Controller] Данные для анимации получены. Ключи: {list(data.keys()) if isinstance(data, dict) else 'Не словарь'}")
        # Добавим проверку на наличие необходимых данных
        if isinstance(data, dict) and 'x' in data and 'y' in data:
            print(f"[Controller] Данные содержат x ({len(data['x'])} точек) и y ({len(data['y'])} точек)")
        else:
            print("[Controller] ОШИБКА: Данные для анимации некорректны!")
