...
def _method_plot_style(label: str) -> dict:
    s = str(label)
    if "RK4_ref" in s or "RK4 ref" in s:
        return {"linestyle": "-", "linewidth": 2.2, "alpha": 0.95, "zorder": 4}
    if "Euler" in s:
        return {"linestyle": "-", "linewidth": 1.6, "alpha": 0.9, "zorder": 1}
    if "Heun" in s:
        return {"linestyle": "--", "linewidth": 1.5, "alpha": 0.95, "zorder": 2}
    if "RK4" in s:
        return {"linestyle": ":", "linewidth": 2.0, "alpha": 0.95, "zorder": 3}
    return {"linestyle": "-", "linewidth": 1.2, "alpha": 0.85, "zorder": 0}
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.le_dt_coarse.setValue(0.11)
    ...
    def plot_results(self, results_dict):
        ...
        fig.suptitle("Результаты моделирования", fontsize=14, y=1.02)

        if not results_dict:
            fig.canvas.draw()
            return

        def plot_series(ax, xdata, ydata, use_legend_labels: bool):
            for label, data in results_dict.items():
                st = _method_plot_style(label)
                lbl = label if use_legend_labels else None
                ax.plot(xdata(data), ydata(data), label=lbl, **st)

        ax = axs[0, 0]
        plot_series(ax, lambda d: d["t"], lambda d: d["x"], True)
        ...
        ax = axs[0, 1]
        plot_series(ax, lambda d: d["t"], lambda d: d["y"], False)
        ax.set_xlabel("Время t")
        ...
        ax = axs[1, 0]
        plot_series(ax, lambda d: d["t"], lambda d: d["vx"], False)
        ax.set_xlabel("Время t")
        ...
        ax = axs[1, 1]
        plot_series(ax, lambda d: d["t"], lambda d: d["vy"], False)
        ax.set_xlabel("Время t")
        ...
        ax = axs[2, 0]
        plot_series(ax, lambda d: d["x"], lambda d: d["y"], False)
        ...
        fig.subplots_adjust(top=0.80, hspace=0.38, wspace=0.28)
        fig.canvas.draw()
